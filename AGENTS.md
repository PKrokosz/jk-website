...
- Pierwszy krok po weryfikacji środowiska uruchamia `pnpm db:generate` na repozytorium (wymaga czystego `git status` w `drizzle/`) i kończy się błędem, jeśli po komendzie pojawią się nowe pliki/zmiany – traktuj to jak obowiązkową bramkę.
